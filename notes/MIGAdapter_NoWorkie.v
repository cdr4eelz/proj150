...
module MIGAdapter (
    ...
    assign fifo_caf_rd_en = (current_state == IDLE) && fifo_caf_valid;
    assign app_en = (current_state == FETCH1);
    assign app_addr = hold_addr;
    assign app_cmd = 3'b001; // READ command
    ...
    always @(posedge ui_clk) begin
        if (ui_clk_sync_rst) begin
            ...
            if (fifo_caf_rd_en) begin
                hold_addr <= fifo_caf_dout[27:0];
            end else begin
                hold_addr <= hold_addr;
            end
        end
    ...
    always @(*) begin
        next_state = IDLE;
        case(current_state)
            IDLE   : next_state = (!fifo_caf_rd_en       ) ? IDLE  :
                         ((fifo_caf_dout[30:28] == 3'b000) ? WRITE1 : FETCH1);
            ISSUE  : next_state = (app_wdf_rdy) ? WRITE1  : ISSUE;
            WRITE1 : next_state = (app_wdf_rdy) ? WRITE2  : WRITE1;
            WRITE2 : next_state = (app_wdf_rdy) ? IDLE    : WRITE2;
            FETCH1 : next_state = (               app_rdy) ? IDLE  : FETCH1;
